  ...
  if (error || !worker) return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: spacing.xl, gap: spacing.md }}>
        ...
        <Text style={{ fontSize: fontSize.sm, color: colors.textMuted, textAlign: 'center' }}>{error ?? 'Worker not found'}</Text>
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <LinearGradient colors={[colors.gradientStart, colors.gradientEnd]} style={styles.hero}>
          ...
          <View style={styles.heroContent}>
            ...
            <View style={styles.ratingRow}>
              ...
              <Text style={styles.ratingCount}>({(worker as any).totalReviews ?? worker.reviewCount ?? 0} reviews)</Text>
            </View>
          </View>
        </LinearGradient>
        ...
        <View style={styles.statsRow}>
          {[
            { icon: 'briefcase-outline' as const, val: `${(worker as any).experience ?? worker.experienceYears ?? 0}+`, label: 'Years Exp.' },
            { icon: 'checkmark-circle-outline' as const, val: `${(worker as any).totalJobs ?? 0}+`, label: 'Jobs Done' },
            { icon: 'radio-button-on' as const, val: worker.isOnline ? 'Online' : 'Offline', label: 'Status' },
          ].map((s) => (
        ...
        <View style={styles.content}>
          ...
          <View style={styles.badgeRow}>
            {[
              { icon: 'shield-checkmark-outline' as const, label: 'Background Verified' },
              { icon: 'document-text-outline' as const, label: 'Govt. ID Checked' },
              ...
              <Card key={r.id} style={styles.reviewCard}>
                <View style={styles.reviewHeader}>
                  <View style={styles.reviewAvatar}>
                    <Text style={styles.reviewInitial}>{(r.user?.name ?? r.customerName ?? 'C')[0]}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.reviewName}>{r.user?.name ?? r.customerName ?? 'Customer'}</Text>
                  ...
                  <Text style={styles.reviewDate}>{new Date(r.createdAt ?? Date.now()).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</Text>
                </View>
      ...
      <View style={styles.stickyBottom}>
        <Button title="Book This Professional" onPress={() => router.push({ pathname: '/service/list', params: {} })} />
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  backBtn: {
    ...
    marginLeft: spacing.xl, marginBottom: spacing.xl,
  },
  heroContent: { alignItems: 'center', paddingHorizontal: spacing.xl },
  avatar: { width: 96, height: 96, borderRadius: 48, borderWidth: 3, borderColor: 'rgba(255,255,255,0.6)', marginBottom: spacing.md },
  avatarPlaceholder: {
  ...
  workerName: { fontSize: fontSize.xxl, fontWeight: fontWeight.extrabold, color: colors.white, marginBottom: spacing.xs },
  ratingRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs },
  ratingVal: { fontSize: fontSize.md, fontWeight: fontWeight.bold, color: colors.white },
  ...
  statVal: { fontSize: fontSize.lg, fontWeight: fontWeight.extrabold, color: colors.textPrimary },
  ...
  skillText: { fontSize: fontSize.sm, color: colors.primary, fontWeight: fontWeight.semibold },

  badgeRow: { gap: spacing.md },
  badge: { flexDirection: 'row', alignItems: 'center', gap: spacing.md },
  badgeText: { fontSize: fontSize.sm, color: colors.textPrimary, fontWeight: fontWeight.medium },

  noReviews: { color: colors.textMuted, fontSize: fontSize.sm },
  reviewCard: { marginBottom: spacing.md },
  reviewHeader: { flexDirection: 'row', alignItems: 'flex-start', gap: spacing.md, marginBottom: spacing.sm },
  reviewAvatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.primaryLight, alignItems: 'center', justifyContent: 'center' },
  reviewInitial: { fontSize: fontSize.sm, fontWeight: fontWeight.extrabold, color: colors.primary },
  reviewName: { fontSize: fontSize.sm, fontWeight: fontWeight.bold, color: colors.textPrimary, marginBottom: 2 },
